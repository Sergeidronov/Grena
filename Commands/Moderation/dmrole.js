...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    for (var i = 0; i < memberIds.length; i++) {
      ...
      try {
        var sendMessage = await member.send({embeds: [new MessageEmbed().setColor("AQUA").setTitle("New role message 📨").setDescription(`${message}`).addFields({name: "Guild", value: `${interaction.guild.name}`, inline: true}, {name: "Role", value: `${role.name}`, inline: true}, {name: "Message Sender", value: `${interaction.member}`, inline: true}).setFooter({text: "Thank you for using my code. | M4HD1#6336"})]})
        successfulMembers += 1
    ...
    interaction.editReply({content: `${interaction.member}`, embeds: [Embed.setDescription(`**Finished sending dm to all users with the role ${role}**`).addFields({name: "Successful DMs", value: `${successfulMembers}`}, {name: "Failed DMs", value: `${failedMembers}`, inline: true}, {name: "Successful members", value: `${successfulMembersMessage}`}, {name: "Failed members", value: `${failedMembersMessage}`}).setFooter({text: "Thank you for using my code. | M4HD1#6336"})]})
  },
};