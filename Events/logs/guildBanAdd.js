  ...
  async execute(ban, client) {
    ...
    if (log) { // If there is a corresponding entry creates the embed
      const memberBannedLogEmbed = new MessageEmbed()
        .setTitle("<:icons_banmembers:866943415361732628> Пользователь был забанен")
        .setColor("RED")
        .setTimestamp()
        .setFooter(ban.guild.name)
        .setDescription(`> Пользователь \`${log.target.tag}\` был забанен \`${log.executor.tag}\``);

      if (log.reason) memberBannedLogEmbed.addField("Причина:", log.reason) // If there is a reason adds a field for it
      ...
      const memberBannedEmbed = new MessageEmbed()
        ...
        .setTimestamp()
        .setFooter(ban.guild.name)
        .setDescription(`> Пользователь \`${ban.user.tag}\` был забанен`);
