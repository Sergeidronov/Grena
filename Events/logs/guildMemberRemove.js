...
module.exports = {
  ...
  async execute(member) {
    ...
    const memberLeftEmbed = new MessageEmbed()
      .setTitle("<:icons_banmembers:866943415361732628> Пользователь покинул сервер")
      .setColor("RED")
      .setTimestamp()
    ...
    if (log.action == "MEMBER_KICK") { // If the last entry fetched is of the type "MEMBER_KICK" it means the member got prunned out of the server
      memberLeftEmbed.setDescription(`> Пользователь \`${log.target.tag}\` кикнут \`${log.executor.tag}\``)
      if (log.reason) memberLeftEmbed.addField("Причина:", `\`${log.reason}\``)
      ...
      memberLeftEmbed.setDescription(`> Пользователь \`${log.target.tag}\` был исключен \`${log.executor.tag}\``)
      if (log.reason) memberLeftEmbed.addField("пРИЧИНА:", `\`${log.reason}\``)

      return await createAndDeleteWebhook(memberLeftEmbed); // executes the function bellow with as parameter the embed name
    } else { // Else it means the member left normally
      memberLeftEmbed.setDescription(`> The Пользователь \`${member.user.tag}\` покинул сервер`)
