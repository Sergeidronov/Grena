...
module.exports = {
  ...
  async execute(channel, client) {
    ...
    const channelPinsChangeEmbed = new MessageEmbed()
      .setTitle("<:icons_updatemember:866943416256167936> Канал был обновлен")
      .setTimestamp()
    ...
    if (log.action == "MESSAGE_PIN") { // If the last entry fetched is of the type "MESSAGE_PIN" executes the code
      channelPinsChangeEmbed.setColor("GREEN").setDescription(`> Сообщение от \`${log.target.tag}\` было закреплено в ${channel} от \`${log.executor.tag}\``)
    }

    if (log.action == "MESSAGE_UNPIN") {// If the last entry fetched is of the type "MESSAGE_UNPIN" executes the code
      channelPinsChangeEmbed.setColor("RED").setDescription(`> Сообщение от \`${log.target.tag}\` было откреплено в ${channel} от \`${log.executor.tag}\``)
    }
